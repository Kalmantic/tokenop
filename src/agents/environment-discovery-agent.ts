...
export class EnvironmentDiscoveryAgent {

  /**
   * Stage 2: Environment Discovery ⏳
   * Discover the full LLM stack
   */
  async discoverEnvironment(): Promise<EnvironmentProfile> {
    ...
    try {
      ...
      console.error("Stage 2: Environment Discovery ❌", (error as Error).message);
   ...
   */
  private async discoverApplicationLayer(environment: EnvironmentProfile): Promise<void> {
    ...
    try {
      ...
      // Detect runtimes
      if (hasOpenAI) environment.application.runtime_detected.push('openai');
      if (hasHuggingFace) environment.application.runtime_detected.push('huggingface');
      if (hasLangChain) environment.application.runtime_detected.push('langchain');
      ...
      if (hasNodeOpenAI) environment.application.runtime_detected.push('nodejs');
   ...
   */
  private async discoverServingLayer(environment: EnvironmentProfile): Promise<void> {
    ...
    try {
      ...
      if (hasONNX) environment.serving.model_formats.push('onnx');
      if (hasPyTorch) environment.serving.model_formats.push('pytorch');
   ...
   */
  private async discoverInfrastructureLayer(environment: EnvironmentProfile): Promise<void> {
    ...
    try {
      ...
      const totalMonthlyCost = environment.infrastructure.gpu_inventory.reduce(
        (sum, gpu) => sum + (gpu.cost_per_hour * 24 * 30), 0
      );
   ...
   */
  private async detectGPUs(environment: EnvironmentProfile): Promise<void> {
    try {
      ...
      const totalMemory = environment.infrastructure.gpu_inventory.reduce(
        (sum, gpu) => sum + gpu.memory_gb, 0
      );
   ...
   */
  private async searchInFiles(files: string[], patterns: string[]): Promise<boolean> {
   ...
   */
  private getGPUBandwidth(name: string): number {
    const lowerName = name.toLowerCase();
    if (lowerName.includes('h100')) return 3350;
    if (lowerName.includes('a100')) return 2000;
    if (lowerName.includes('v100')) return 900;
    if (lowerName.includes('rtx')) return 1000;
    return 800; // Default
  }
   ...
   */
  private getGPUCost(name: string): number {
    const lowerName = name.toLowerCase();
    if (lowerName.includes('h100')) return 4.0;
    if (lowerName.includes('a100')) return 3.0;
    if (lowerName.includes('v100')) return 2.0;
    if (lowerName.includes('rtx')) return 1.5;
    return 2.0; // Default
  }
}